...
public:	
	...
	void popfront() {
		NodeQ<T> * nxt = head->spine;
		head = nxt;
		size--;
	}

	void insert(NodeQ<T> * newNode) {
		if (size == 0) {
			head = newNode;
			size++;
			return;
	...
	void insert(int newNode) {
		...
		temp->spine = new NodeQ<int>(newNode);
		size++;
	}

	NodeQ<T> * search(int identity) {
		NodeQ<T> * prev = head;
		NodeQ<T> * tip = prev->spine;
		NodeQ<T> * nxt;
		...
		while (tip != NULL) {
			...
			cout << "A" << endl;
			while (nxt != NULL) {
				if (nxt->index == identity) {
					prev->spine = tip->spine;
					size--;
					return tip;
				}
				cout << "B" << endl;
